...
Each level contains 20 AI-generated lessons:
...
- **Three difficulty levels** — Beginner, Intermediate, and Advanced (20 lessons each, 60 total)
...
The first startup takes 5–10 minutes while the API seeds 60 lessons with AI-generated content and audio.
...
GET /health/ready    → { "status": "ready", "lessons": 60 }
                     → { "status": "seeding", "lessons": 3, "expected": 60 }
```
...
Lessons are automatically seeded on first API startup (60 lessons total: 20 per level). To manually re-seed:
